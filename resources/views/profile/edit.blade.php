...
@section('content')
    <div class="mx-auto max-w-4xl py-6 sm:py-8">
        <div class="mb-8">
            <div class="badge-soft">Profile settings</div>
            <h1 class="mt-4 text-3xl font-semibold tracking-tight text-slate-900 sm:text-4xl">
                Edit Your Profile
            </h1>
            <p class="mt-3 max-w-2xl text-sm leading-7 text-slate-600 sm:text-base">
                Update your contact details, blood group, donor status, and location information.
            </p>
        </div>

        @if (session('status'))
            <div class="mb-6 rounded-[28px] border border-emerald-200 bg-emerald-50/90 p-5 text-emerald-800 shadow-sm">
        ...
        <div class="card-soft p-5 sm:p-7">
            <form method="POST" action="{{ route('profile.update') }}" class="space-y-6">
                ...
                <input type="hidden" name="location_mode" id="location_mode"
                    value="{{ old('location_mode', $locationMode ?? (!empty($user->city_area_id) ? 'city' : 'upazila')) }}">
                ...
                <section class="rounded-[26px] border border-rose-100 bg-rose-50/50 p-5 sm:p-6">
                    <div class="flex items-center justify-between gap-3">
                        <div>
                            ...
                            <p class="mt-1 text-sm text-slate-500">The essentials people may need to contact and match you.
                            </p>
                        </div>

                        <div
                            class="hidden sm:block rounded-full bg-white px-3 py-1 text-xs font-semibold text-red-700 ring-1 ring-rose-100">
                            Profile
                        </div>
                    </div>
                    ...
                    <div class="mt-5 grid grid-cols-1 gap-4 sm:grid-cols-2">
                        <div>
                            ...
                            <input name="phone" value="{{ old('phone', $user->phone) }}" class="mt-2"
                        ...
                        <div>
                            ...
                            <select name="blood_group" class="mt-2" required>
                                ...
                                @php
                                    ...
                                    $selectedGroup = old('blood_group', $user->blood_group);
                                @endphp
                                @foreach($groups as $g)
                                    <option value="{{ $g }}" @selected($selectedGroup === $g)>{{ $g }}</option>
                                @endforeach
                            </select>
                ...
                <section class="rounded-[26px] border border-slate-100 bg-white p-5 sm:p-6">
                    <div class="flex items-center justify-between gap-3">
                        ...
                        <div
                            class="hidden sm:block rounded-full bg-amber-50 px-3 py-1 text-xs font-semibold text-amber-700 ring-1 ring-amber-200">
                            Required
                        </div>
                    </div>
                    ...
                    <div id="dhakaModeWrapper" class="hidden mt-5 rounded-2xl border border-rose-100 bg-rose-50/50 p-4">
                        ...
                        <div class="mt-4 flex flex-wrap gap-3">
                            <label
                                class="inline-flex items-center gap-2 rounded-full border border-rose-100 bg-white px-4 py-2 text-sm text-slate-700 shadow-sm">
                                <input type="radio" name="dhaka_mode_radio" value="upazila"
                ...
                <section class="rounded-[26px] border border-slate-100 bg-white p-5 sm:p-6">
                    <div class="flex items-center justify-between gap-3">
                        ...
                        <div
                            class="hidden sm:block rounded-full bg-slate-50 px-3 py-1 text-xs font-semibold text-slate-600 ring-1 ring-slate-200">
                            Optional
                        </div>
                    </div>
                    ...
                    <div class="mt-5 space-y-5">
                        <div>
                            ...
                            <input name="address_line" value="{{ old('address_line', $user->address_line) }}" class="mt-2"
                        ...
                        <div>
                            ...
                            <textarea name="medical_history" rows="4" class="mt-2"
                        ...
                        <div class="rounded-2xl border border-rose-100 bg-rose-50/50 p-4">
                            <label class="flex items-start gap-3">
                                <input type="checkbox" id="become_donor" name="become_donor" value="1"
                                    ...
                                    @checked(old('become_donor', $donorProfile ? 1 : 0))>
                                <div>
                            ...
                            <div id="donorDateWrapper" class="hidden mt-4">
                                ...
                                <input type="date" name="last_donate_date"
                                    value="{{ old('last_donate_date', optional($donorProfile?->last_donate_date)->format('Y-m-d') ?? $donorProfile?->last_donate_date) }}"
                                    class="mt-2">
                ...
                <div
                    class="flex flex-col-reverse gap-3 border-t border-rose-100 pt-6 sm:flex-row sm:items-center sm:justify-end">
    ...
    <script>
        (function () {
            ...
            const oldDivision = @json(old('division_id', $user->division_id));
            const oldDistrict = @json(old('district_id', $user->district_id));
            const oldUpazila = @json(old('upazilla_id', $user->upazilla_id));
            const oldMode = @json(old('location_mode', $locationMode ?? (!empty($user->city_area_id) ? 'city' : 'upazila')));
            const oldCorp = @json(old('city_corporation_id', $user->city_corporation_id));
            const oldCityArea = @json(old('city_area_id', $user->city_area_id));
            ...
            function fillSelect(selectEl, items, placeholder, selectedValue = null) {
            ...
            async function loadDistricts(divisionId, selectedDistrict = null) {
                fillSelect(districtEl, [], 'Select District');
                fillSelect(upazilaEl, [], 'Select Upazila');
                fillSelect(corpEl, [], 'Select City Corporation');
                fillSelect(cityAreaEl, [], 'Select City Area');
                ...
                fillSelect(districtEl, data, 'Select District', selectedDistrict);
            }

            async function loadUpazilas(districtId, selectedUpazila = null) {
                fillSelect(upazilaEl, [], 'Select Upazila');
                if (!districtId) return;

                const res = await fetch(`/locations/districts/${districtId}/upazillas`);
                const data = await res.json();
                fillSelect(upazilaEl, data, 'Select Upazila', selectedUpazila);
            }

            async function loadDhakaCorps(selectedCorp = null) {
                fillSelect(corpEl, [], 'Select City Corporation');
                fillSelect(cityAreaEl, [], 'Select City Area');

                const res = await fetch(`/locations/dhaka/city-corporations`);
                const corps = await res.json();
                fillSelect(corpEl, corps, 'Select City Corporation', selectedCorp);
            }

            async function loadCityAreas(corpId, selectedArea = null) {
                fillSelect(cityAreaEl, [], 'Select City Area');
                if (!corpId) return;

                const res = await fetch(`/locations/dhaka/city-corporations/${corpId}/areas`);
                const areas = await res.json();
                fillSelect(cityAreaEl, areas, 'Select City Area', selectedArea);
            }
            ...
            districtEl?.addEventListener('change', async () => {
                ...
                locationModeEl.value = 'upazila';
                const upazilaRadio = document.querySelector('input[name="dhaka_mode_radio"][value="upazila"]');
                if (upazilaRadio) upazilaRadio.checked = true;
            ...
            document.querySelectorAll('input[name="dhaka_mode_radio"]').forEach(radio => {
                radio.addEventListener('change', async (e) => {
                    ...
                    if (mode === 'city') {
                        ...
                        await loadDhakaCorps(oldCorp);
                        if (corpEl.value) {
                            await loadCityAreas(corpEl.value, oldCityArea);
                        }
                    } else {
                        setVisible(upazilaWrapper, true);
                        setVisible(corpWrapper, false);
            ...
            (async function init() {
                if (oldDivision) {
                    await loadDistricts(oldDivision, oldDistrict);
                }

                if (oldDistrict) {
                    await loadUpazilas(oldDistrict, oldUpazila);
                }
                ...
                if (isDhaka) {
                    const selectedMode = oldMode === 'city' ? 'city' : 'upazila';
                    locationModeEl.value = selectedMode;

                    const modeRadio = document.querySelector(`input[name="dhaka_mode_radio"][value="${selectedMode}"]`);
                    ...
                    if (selectedMode === 'city') {
                        ...
                        await loadDhakaCorps(oldCorp);
                        if (oldCorp) {
                            await loadCityAreas(oldCorp, oldCityArea);
                        }
                    } else {
                        setVisible(upazilaWrapper, true);
                        setVisible(corpWrapper, false);
